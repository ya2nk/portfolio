...
class imgbrowse {
    ...
    protected $root     = '/industri/public/upload/ckeditor/';
  ...
  // return two-dimensional array with folders-list and images in specified $imgdr
    public function getMenuImgs() {
        ...
        $site       = $protocol. $_SERVER['SERVER_NAME']. (!empty($port) ? ':'.$port : '') .'/';
        ...
        foreach($obdr as $fileobj) {
            ...
            if($fileobj->isFile() && in_array($fileobj->getExtension(), $this->imgext))
                $re['imgs'] .= '<span><img src="'. $site . $this->root . $this->imgdr . $name .'" alt="'. $name .'" height="70" title="Pilih" />'. $name .'</span>';
