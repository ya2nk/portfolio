            ...
            <div class="modal-body">
                <form id="ajaxForm" name="ajaxForm" class="form-horizontal">
                    ...
                    <div class="form-group">
			            <label for="title">Knowledges</label>
...
<script>
    $(function () {
        ...
        $('#createNew').click(function () {
                $('#saveBtn').val("create-knowledges");
                $('#id').val('');
                $('#ajaxFrom').trigger("reset");
                $('#titleModal').html("Create Knowledges");
                $('#ajaxModal').modal('show');
        });

        $('body').on('click', '.editData', function () {
                var id = $(this).data('id');
                $.get(' knowledges/' + id + '/edit', function (data) {
                    $('#titleModal').html("Edit " + data.title);
                    $('#saveBtn').val("edit-knowledges");
                    $('#ajaxModal').modal('show');
                    $('#id').val(data.id);
        ...
        $('#ajaxForm').submit(function (e) {
            ...
            $.ajax({
                ...
                success: (data) => {
                    ...
                    toast({
                        type: 'success',
                        title: data.title + ' data is successfully saved',
                        padding: '2em',
                    })
                },
                error: function (data) {
                    ...
                    toast({
                        type: 'error',
                        title: ' Knowledges must be filled',
                        padding: '2em',
                    })
                    $('#saveBtn').html('Save');
                }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        setTimeout(function () {
                            ...
                            toast({
                                type: 'success',
                                title: ' Knowledges is successfully delete',
                                padding: '2em',
                            })
                        });
